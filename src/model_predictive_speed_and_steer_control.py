...
def get_linear_model_matrix(v, phi, delta):
    ...
    # tau = 2.0
    # delta = state.steering_angle + DT / tau * \
    # (steering_command - state.steering_angle)
...
def update_state(state, a, steering_command):

    # input check

    delta = state.steering_angle + DT / state.tau * \
        (steering_command - state.steering_angle)
...
def predict_motion(x0, oa, od, xref):
    ...
    state = State(x=x0[0], y=x0[1], yaw=x0[3], v=x0[2], steering_angle=x0[4])
...
def calc_ref_trajectory(state, cx, cy, cyaw, ck, sp, dl, pind):
    """calculate the forward trajectory in several steps
...
def do_simulation(cx, cy, cyaw, ck, sp, dl, initial_state):
    ...
    while MAX_TIME >= time:
        ...
        x0 = [state.x, state.y, state.v, state.yaw,
              state.steering_angle]  # current state
